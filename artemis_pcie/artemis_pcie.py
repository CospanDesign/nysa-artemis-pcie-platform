...
import artemis_pcie_utils


ARTEMIS_PCIE_QUEUE_TIMEOUT = 7
ARTEMIS_PCIE_PING_TIMEOUT = 1
ARTEMIS_PCIE_WRITE_TIMEOUT = 5
ARTEMIS_PCIE_READ_TIMEOUT = 3
...
ARTEMIS_PCIE_RESET = 1
ARTEMIS_PCIE_WRITE = 2
ARTEMIS_PCIE_READ = 3
ARTEMIS_PCIE_PING = 4
ARTEMIS_PCIE_DUMP_CORE = 5
ARTEMIS_PCIE_IS_PROGRAMMED = 6

ARTEMIS_PCIE_RESP_OK = 0
ARTEMIS_PCIE_RESP_ERR = -1

ARTEMIS_PCIE_MEMORY_OFFSET = 0x0100000000

_artemis_pcie_instances = {}



def ArtemisPCIE(idVendor = 0x0403, idProduct = 0x8532, sernum = None, status = False):
    global _artemis_pcie_instances
    if sernum in _artemis_pcie_instances:
        return _artemis_pcie_instances[sernum]
    _artemis_pcie_instances[sernum] = _ArtemisPCIE(idVendor, idProduct, sernum, status)
    return _artemis_pcie_instances[sernum]

class ArtemisPCIEData(object):
    data = None
    length = 0
...
class WorkerThread(threading.Thread):
    ...
    def run(self):
        ...
        while (1):
            try:
                ...
                if wdata == ARTEMIS_PCIE_RESET:
                    self.reset()
                elif wdata == ARTEMIS_PCIE_IS_PROGRAMMED:
                    self.is_programmed()
                elif wdata == ARTEMIS_PCIE_PING:
                    self.ping()
                elif wdata == ARTEMIS_PCIE_WRITE:
                    self.write()
                elif wdata == ARTEMIS_PCIE_READ:
                    self.read()
                elif wdata == ARTEMIS_PCIE_DUMP_CORE:
                    self.dump_core()
                else:
    ...
    def reset(self):
        ...
        self.hrq.put(ARTEMIS_PCIE_RESP_OK)
    ...
    def write(self):
        ...
        if len(rsp) > 0 and rsp[0] == 0xDC:
            ...
            timeout = time.time() + ARTEMIS_PCIE_WRITE_TIMEOUT
        ...
        if len(rsp) > 0:
            if rsp[0] != 0xDC:
                ...
                self.hrq.put(ARTEMIS_PCIE_RESP_ERR)
                return
    ...
    def read(self):
        ...
        if len(rsp) > 1 and rsp[0] == 0xDC and rsp[1] == 0xFD:
            ...
            timeout = time.time() + ARTEMIS_PCIE_READ_TIMEOUT
            while time.time() < timeout:
                rsp = self.dev.read_data_bytes(1)
                if len(rsp) > 0 and rsp[0] == 0xDC:
                    #Got an ack
    ...
    def ping(self):
        ...
        if len(rsp) > 0 and rsp[0] == 0xDC:
            ...
            timeout = time.time() + ARTEMIS_PCIE_PING_TIMEOUT
            while time.time() < timeout:
                rsp = self.dev.read_data_bytes(1)
                if len(rsp) > 0 and rsp[0] == 0xDC:
                    #Got a response
        ...
        if len(rsp) > 0:
            if rsp[0] != 0xDC:
                self.hrq.put(ARTEMIS_PCIE_REP_ERR)
                return
        else:
            self.hrq.put(ARTEMIS_PCIE_RESP_ERR)
            return
...
class _ArtemisPCIE (Nysa):
    ...
    def __init__(self, idVendor = 0x0403, idProduct = 0x8532, sernum = None, status = False):
    ...
    def ipc_comm_response(self, name):
        try:
            resp = self.hrq.get(block = True, timeout = ARTEMIS_PCIE_QUEUE_TIMEOUT)
            if resp == ARTEMIS_PCIE_RESP_OK:
                #print "%s got an OK response!" % current_thread().name
                return self.d.data
            else:
                raise NysaCommError("Artemis response error %s: %d" % (name, resp))
        except Queue.Empty:
            raise NysaCommError("Artemis error %s: timeout: %d" % (name, ARTEMIS_PCIE_QUEUE_TIMEOUT))
    ...
    def read(self, address, length = 1, disable_auto_inc = False):
        ...
        with self.lock:
            ...
            if address >= ARTEMIS_PCIE_MEMORY_OFFSET:
                address -= ARTEMIS_PCIE_MEMORY_OFFSET
            ...
            self.hwq.put(ARTEMIS_PCIE_READ)
    ...
    def write(self, address, data, disable_auto_inc = False):
        ...
        with self.lock:
            ...
            self.hwq.put(ARTEMIS_PCIE_WRITE)
    ...
    def ping (self):
        ...
        """
        with self.lock:
            self.hwq.put(ARTEMIS_PCIE_PING)
    ...
    def reset (self):
        ...
        with self.lock:
            ...
            self.hwq.put(ARTEMIS_PCIE_RESET)
    ...
    def is_programmed(self):
        ...
        with self.lock:
            self.d.data = (self.vendor, self.product)
            self.hwq.put(ARTEMIS_PCIE_IS_PROGRAMMED)
            self.dev.purge_buffers()
    ...
    def dump_core(self):
        ...
        """
        with self.lock:
            self.hwq.put(ARTEMIS_PCIE_DUMP_CORE)
    ...
    def upload(self, filepath):
        artemis_pcie_utils.upload(self.vendor, self.product, self.sernum, filepath, self.s)

    def program (self):
        artemis_pcie_utils.program(self.vendor, self.product, self.sernum, self.s)
