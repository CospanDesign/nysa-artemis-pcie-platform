...
@cocotb.coroutine
def setup_dut(dut):
    dut.s1.api.pcie_interface.user_reset_out <= 1;
    cocotb.fork(Clock(dut.s1.api.pcie_interface.user_clk_out, PCIE_CLK_PERIOD).start())
    for i in range(20):
        yield RisingEdge(dut.s1.api.pcie_interface.user_clk_out)
        yield ReadOnly()
...
def test_simple_nysa_write_command(dut):
    ...
    dut.log.info("Get Control")
    v = yield cocotb.external(driver.get_control)()

    dut.log.info("Configure PCIE")
    c.configure_FPGA()
